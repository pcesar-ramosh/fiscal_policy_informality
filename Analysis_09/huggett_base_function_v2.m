function out = huggett_base_function_v2(params)
% -------------------------------------------------------------------------
% Huggett biesector con 2 tipos (Informal / Formal), impuestos y transferencias,
% prima de endeudamiento informal, bien público en utilidad y DEUDA PÚBLICA
% configurable:
%   debt.mode:
%     - 'identity'    (legacy)  -> B = (Tl+Tc - (G+Tr))/r  => Ingresos = Gastos
%     - 'exog_level'            -> B = debt.B_level (constante)
%     - 'exog_ratio'            -> B = debt.bY_ratio * Y   (porcentaje del PIB)
%
% Devuelve: r, a, g, c, s, popI/popF, cuentas fiscales con PB y BB (no forzadas),
% estadísticas (medias, medianas, Gini por tipo y total), prestatarios/prestamistas.
% -------------------------------------------------------------------------

%% ----- utilidades para defaults
...
tau_l = arg('tau_l', 0.15);   % impuesto laboral (solo formales)
tau_c = arg('tau_c', 0.10);   % IVA
phi   = arg('phi',   0.15);   % transferencias a informales
Gov   = arg('Gov',   0.07);   % bien público per cápita
...
if useCustomLambda
    la1_fun = params.lambdaSpec.la1_fun;  % la1_fun(a, eta)
    la2_fun = params.lambdaSpec.la2_fun;  % la2_fun(a, eta)
end

% Deuda pública
debt = struct('mode','identity','B_level',0,'bY_ratio',0.40);
if isfield(params,'debt')
    if isfield(params.debt,'mode');     debt.mode     = params.debt.mode;     end
    if isfield(params.debt,'B_level');  debt.B_level  = params.debt.B_level;  end
    if isfield(params.debt,'bY_ratio'); debt.bY_ratio = params.debt.bY_ratio; end
end
...
for ir = 1:Ir
    ...
    % Deuda según modo
    switch lower(debt.mode)
        case 'identity'
            B = (Tl + Tc - (Gx + Tr)) / max(r,1e-9);  % legacy
        case 'exog_level'
            B = debt.B_level;
        case 'exog_ratio'
            B = debt.bY_ratio * Y;
        otherwise
            B = (Tl + Tc - (Gx + Tr)) / max(r,1e-9);
    end

    % Exceso de activos para bisección
    S = (g(:,1)'*a + g(:,2)'*a)*da - B;
    ...
    % actualizar v0 con nueva r
    rr = r*ones(I,1); rr(a<0) = r + theta;
    v0(:,1) = ((((1-0)*zz(:,1) + rr.*a + phi*z1)/(1+tau_c)).^(1-sI)/(1-sI) + Gov)/rho;
    v0(:,2) = ((((1-tau_l)*zz(:,2) + r .*a          )/(1+tau_c)).^(1-sF)/(1-sF) + Gov)/rho;

    % si λ depende de popI, refrescar Aswitch (opcional)
    if useCustomLambda
...
% Cuentas fiscales finales (no forzadas si modo exógeno)
rB = r * B;
PB = Tl + Tc - (Gx + Tr);
BB = PB - rB;                    % ≠ 0 si B es exógena
gapBudget = (Tl + Tc) - (Gx + Tr + rB);

% Estadísticas riqueza y consumo (medias, medianas, Gini)
...
gini_I  = giniW(a, g(:,1), da);  gini_F  = giniW(a, g(:,2), da);
...
idxB = (a<0); idxL = (a>0);
fracBorrow_I = sum(g(idxB,1))*da / max(popI,1e-12);
fracBorrow_F = sum(g(idxB,2))*da / max(popF,1e-12);
fracLend_I   = sum(g(idxL,1))*da / max(popI,1e-12);
fracLend_F   = sum(g(idxL,2))*da / max(popF,1e-12);
...
out.fiscal = struct('mode',debt.mode,'Tl',Tl,'Tc',Tc,'Tr',Tr,'G',Gx, ...
                    'B',B,'rB',rB,'PB',PB,'BB',BB,'gapBudget',gapBudget);

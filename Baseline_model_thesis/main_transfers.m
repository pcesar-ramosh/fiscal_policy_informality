%% ============================ FILE: scenario_phi_compare.m ============================
% Compares steady states for LOWER, BASELINE, and HIGHER transfers to informals (phi)
% Reuses solve_two_type_huggett_fiscal.m. Exports paper-ready figures & a summary CSV.
%
% OUTPUTS
%   ./figures/compare_phi_*   (PNG+PDF)
%   ./tables/compare_phi_summary.csv
...
base.tau_l = 0.15;                 % keep taxes at baseline
base.tau_c = 0.18;
base.Gov   = 0.05;
base.phi   = 0.09;                 % baseline transfers (will be overwritten)
...
base.eta_target = 0.654;           % same informality target as BASE paper

% Three transfer-policy scenarios (adjust if you prefer other values)
phi_low  = 0.05;
phi_mid  = 0.09;   % baseline
phi_high = 0.13;

cfgL = base; cfgL.phi = phi_low;
cfgM = base; cfgM.phi = phi_mid;
cfgH = base; cfgH.phi = phi_high;
...
labL = sprintf('Lower transfers (phi=%.2f)', phi_low);
labM = sprintf('Baseline (phi=%.2f)',        phi_mid);
labH = sprintf('Higher transfers (phi=%.2f)', phi_high);

% ---------- 3) Summary CSV ----------
S = table;
S.scenario = ["phi_low"; "phi_mid"; "phi_high"];
S.phi      = [phi_low;   phi_mid;   phi_high];
S.r        = [L.r; M.r; H.r];
...
writetable(S, fullfile('./tables','compare_phi_summary.csv'));

% ---------- 4) Policy functions (consumption & savings) ----------
fig = figure('Name','Policy: Consumption vs transfers');
subplot(1,2,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_phi_policy_consumption'));

fig = figure('Name','Policy: Savings vs transfers');
subplot(1,2,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_phi_policy_savings'));

% ---------- 5) Wealth distributions g(a) ----------
fig = figure('Name','Wealth density vs transfers');
subplot(2,1,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_phi_wealth_distributions'));
...
fig = figure('Name','Asset market closure vs transfers');
bar(Mmat); grid on; ylabel('Assets level');
set(gca,'XTickLabel',{'Private demand','Public supply B(r)'});
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_phi_asset_market'));

% ---------- 7) Fiscal accounts ----------
fig = figure('Name','Fiscal accounts vs transfers');
subplot(1,2,1);
...
title('Expenditures');
export_fig(fig, fullfile(outdir_figs,'compare_phi_fiscal_accounts'));

% ---------- 8) Borrowers/Lenders ----------
catX = categorical({'Informal','Formal'}); catX = reordercats(catX,{'Informal','Formal'});
fig = figure('Name','Borrowers-Lenders shares vs transfers');
subplot(1,2,1);
...
export_fig(fig, fullfile(outdir_figs,'compare_phi_borrowers_lenders_shares'));

fig = figure('Name','Borrowers-Lenders volumes vs transfers');
subplot(1,2,1);
...
export_fig(fig, fullfile(outdir_figs,'compare_phi_borrowers_lenders_volumes'));

% ---------- 9) Lorenz curves ----------
[~, LwL, cumPopL] = lorenz_from_density(a, L.g);
[~, LwM, cumPopM] = lorenz_from_density(a, M.g);
[~, LwH, cumPopH] = lorenz_from_density(a, H.g);
fig = figure('Name','Lorenz wealth vs transfers');
...
export_fig(fig, fullfile(outdir_figs,'compare_phi_lorenz_wealth'));
...
fig = figure('Name','Lorenz consumption vs transfers');
...
export_fig(fig, fullfile(outdir_figs,'compare_phi_lorenz_consumption'));

% ---------- 10) MPC(a) overlay by type ----------
% Para mantener comparabilidad con los otros ejercicios, usamos el mismo
% "tiny income bump" (no un shock directo a phi). Así medimos MPC(a) alrededor
% de cada equilibrio con su phi correspondiente.
DO_MPC = true;
if DO_MPC
    eps_z = 0.01;
    % low-phi equilibrium
    ...
    dres1L = (base.z1*eps_z) * (1 + phi_low);      % ingreso informal + transfer
    dres2L = (base.z2*eps_z) * (1 - base.tau_l);   % ingreso formal neto de tau_l
    ...
    dres1M = (base.z1*eps_z) * (1 + phi_mid);
    dres2M = dres2L;
    MPC1M = (solPM.c(:,1) - M.c(:,1)) / max(dres1M,1e-12);
    MPC2M = (solPM.c(:,2) - M.c(:,2)) / max(dres2M,1e-12);

    % high-phi equilibrium
    ...
    export_fig(fig, fullfile(outdir_figs,'compare_phi_mpc_by_assets'));
end
...
if DO_S_CURVE
    ...
    for i=1:numel(rgridL)
        alt.phi = phi_low;  alt.eta_target = base.eta_target;
    ...
    for i=1:numel(rgridM)
        alt.phi = phi_mid;  alt.eta_target = base.eta_target;
    ...
    for i=1:numel(rgridH)
        alt.phi = phi_high; alt.eta_target = base.eta_target;
    ...
    fig = figure('Name','Excess supply S(r) vs transfers');
    ...
    legend({labL,labM,labH},'Location','best');
    export_fig(fig, fullfile(outdir_figs,'compare_phi_excess_supply_curve'));
end
...
fig = figure('Name','Bond demand vs supply (by interest rate, transfers)');
tiledlayout(1,2);
...
title(labL);
legend({'A_{priv}(low phi)','B(low phi)','A_{priv}(base)','B(base)','r^*_{low}','B^*_{low}','Equilibrium'},'Location','best');
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_{priv}(high phi)','B(high phi)','A_{priv}(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');

export_fig(fig, fullfile(outdir_figs,'compare_phi_bond_demand_supply'));
...
fig = figure('Name','Bond demand by type vs public supply (by r, transfers)');
tiledlayout(1,2);
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labL);
legend({'A_I(low phi)','A_F(low phi)','B(low phi)', ...
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_I(high phi)','A_F(high phi)','B(high phi)', ...
        'A_I(base)','A_F(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');

export_fig(fig, fullfile(outdir_figs,'compare_phi_bond_demand_by_type'));

...
% Compares steady states for LOWER, BASELINE, and HIGHER labor income tax (tau_l)
...
base.eta_target = 0.654;           % keep same informality target as your BASE paper

% Three labor-tax scenarios
tau_low  = 0.10;
tau_mid  = 0.15;   % baseline
tau_high = 0.25;

cfgL = base; cfgL.tau_l = tau_low;
cfgM = base; cfgM.tau_l = tau_mid;
...
labM = sprintf('Baseline (tau_l=%.2f)',        tau_mid);
labH = sprintf('Higher labor tax (tau_l=%.2f)', tau_high);

% ---------- 3) Summary CSV ----------
S = table;
S.scenario = ["tau_low"; "tau_mid"; "tau_high"];
S.tau_l    = [tau_low;  tau_mid;  tau_high];
S.r        = [L.r; M.r; H.r];
...
if DO_MPC
    ...
    dres1M = dres1L;
    dres2M = (base.z2*eps_z) * (1 - tau_mid);
    ...
    dres1H = dres1L;
    dres2H = (base.z2*eps_z) * (1 - tau_high);
...
if DO_S_CURVE
    ...
    for i=1:numel(rgridM)
        alt.tau_l = tau_mid;  alt.eta_target = base.eta_target;
...
title(labL);
legend({'A_{priv}(low)','B(low)','A_{priv}(base)','B(base)','r^*_{low}','B^*_{low}','Equilibrium'},'Location','best');
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_{priv}(high)','B(high)','A_{priv}(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labL);
legend({'A_I(low)','A_F(low)','B(low)','A_I(base)','A_F(base)','B(base)','r^*_{low}','B^*_{low}','Equilibrium'},'Location','best');
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_I(high)','A_F(high)','B(high)','A_I(base)','A_F(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');

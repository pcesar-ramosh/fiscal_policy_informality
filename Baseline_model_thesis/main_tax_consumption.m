%% ============================ FILE: scenario_tauc_compare.m ============================
% Compares steady states for LOWER, BASELINE, and HIGHER consumption tax (tau_c)
% Reuses solve_two_type_huggett_fiscal.m. Exports paper-ready figures & a summary CSV.
%
% OUTPUTS
%   ./figures/compare_tauc_*   (PNG+PDF)
%   ./tables/compare_tauc_summary.csv
...
base.tau_l = 0.15;                 % keep labor tax at baseline
base.tau_c = 0.18;                 % baseline VAT (will be overwritten per scenario)
...
% Three consumption-tax scenarios
tauc_low  = 0.10;
tauc_mid  = 0.18;   % baseline
tauc_high = 0.26;

cfgL = base; cfgL.tau_c = tauc_low;
cfgM = base; cfgM.tau_c = tauc_mid;
cfgH = base; cfgH.tau_c = tauc_high;
...
labL = sprintf('Lower VAT (tau_c=%.2f)', tauc_low);
labM = sprintf('Baseline (tau_c=%.2f)',  tauc_mid);
labH = sprintf('Higher VAT (tau_c=%.2f)', tauc_high);

% ---------- 3) Summary CSV ----------
S = table;
S.scenario = ["tauc_low"; "tauc_mid"; "tauc_high"];
S.tau_c    = [tauc_low;  tauc_mid;  tauc_high];
S.r        = [L.r; M.r; H.r];
...
writetable(S, fullfile('./tables','compare_tauc_summary.csv'));

% ---------- 4) Policy functions (consumption & savings) ----------
fig = figure('Name','Policy: Consumption vs VAT');
subplot(1,2,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_tauc_policy_consumption'));

fig = figure('Name','Policy: Savings vs VAT');
subplot(1,2,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_tauc_policy_savings'));

% ---------- 5) Wealth distributions g(a) ----------
fig = figure('Name','Wealth density vs VAT');
subplot(2,1,1);
...
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_tauc_wealth_distributions'));
...
fig = figure('Name','Asset market closure vs VAT');
bar(Mmat); grid on; ylabel('Assets level');
set(gca,'XTickLabel',{'Private demand','Public supply B(r)'});
legend({labL,labM,labH},'Location','best');
export_fig(fig, fullfile(outdir_figs,'compare_tauc_asset_market'));

% ---------- 7) Fiscal accounts ----------
fig = figure('Name','Fiscal accounts vs VAT');
subplot(1,2,1);
...
title('Expenditures');
export_fig(fig, fullfile(outdir_figs,'compare_tauc_fiscal_accounts'));

% ---------- 8) Borrowers/Lenders ----------
catX = categorical({'Informal','Formal'}); catX = reordercats(catX,{'Informal','Formal'});
fig = figure('Name','Borrowers-Lenders shares vs VAT');
subplot(1,2,1);
...
export_fig(fig, fullfile(outdir_figs,'compare_tauc_borrowers_lenders_shares'));

fig = figure('Name','Borrowers-Lenders volumes vs VAT');
subplot(1,2,1);
...
export_fig(fig, fullfile(outdir_figs,'compare_tauc_lorenz_wealth'));
...
fig = figure('Name','Lorenz consumption vs VAT');
...
export_fig(fig, fullfile(outdir_figs,'compare_tauc_lorenz_consumption'));

% ---------- 10) MPC(a) overlay by type ----------
% Keep same shock as before: a tiny income bump; tau_c affects the price of c,
% but we replicate the same measurement for comparability.
DO_MPC = true;
if DO_MPC
    eps_z = 0.01;
    % low-VAT equilibrium
    ...
    export_fig(fig, fullfile(outdir_figs,'compare_tauc_mpc_by_assets'));
end
...
if DO_S_CURVE
    ...
    for i=1:numel(rgridL)
        alt.tau_c = tauc_low;  alt.eta_target = base.eta_target;
    ...
    for i=1:numel(rgridM)
        alt.tau_c = tauc_mid;  alt.eta_target = base.eta_target;
    ...
    for i=1:numel(rgridH)
        alt.tau_c = tauc_high; alt.eta_target = base.eta_target;
    ...
    fig = figure('Name','Excess supply S(r) vs VAT');
    ...
    legend({labL,labM,labH},'Location','best');
    export_fig(fig, fullfile(outdir_figs,'compare_tauc_excess_supply_curve'));
end
...
fig = figure('Name','Bond demand vs supply (by interest rate, VAT)');
tiledlayout(1,2);

% panel: show low vs high, each with baseline dashed as reference
nexttile;
...
title(labL);
legend({'A_{priv}(low VAT)','B(low VAT)','A_{priv}(base)','B(base)','r^*_{low}','B^*_{low}','Equilibrium'},'Location','best');
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_{priv}(high VAT)','B(high VAT)','A_{priv}(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');

export_fig(fig, fullfile(outdir_figs,'compare_tauc_bond_demand_supply'));
...
fig = figure('Name','Bond demand by type vs public supply (by r, VAT)');
tiledlayout(1,2);
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labL);
legend({'A_I(low VAT)','A_F(low VAT)','B(low VAT)', ...
...
grid on; xlabel('Interest rate r'); ylabel('Assets / Debt level');
title(labH);
legend({'A_I(high VAT)','A_F(high VAT)','B(high VAT)', ...
        'A_I(base)','A_F(base)','B(base)','r^*_{high}','B^*_{high}','Equilibrium'},'Location','best');

export_fig(fig, fullfile(outdir_figs,'compare_tauc_bond_demand_by_type'));

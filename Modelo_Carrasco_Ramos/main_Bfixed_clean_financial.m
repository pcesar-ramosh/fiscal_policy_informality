...
%% ===================== Financial market: supply & demand =====================
% Construye curvas A_I(r), A_F(r), A_priv(r) y B(r) en un rango alrededor de r*
% y genera figuras comparables + CSV.

% Rango de tasas alrededor del equilibrio (cauto si r* es pequeño/grande)
r_lo = max(cfg.rmin, max(0.5*r, 0.002));
r_hi = min(cfg.rmax, max(1.8*r, r + 0.02));
if r_hi <= r_lo, r_hi = r_lo*(1+0.05); end
r_span = linspace(r_lo, r_hi, 35);

Apriv_I = nan(size(r_span));
Apriv_F = nan(size(r_span));
Apriv_T = nan(size(r_span));
B_grid  = nan(size(r_span));
Y_grid  = nan(size(r_span));
S_grid  = nan(size(r_span));

% Re-usar el mismo B de tu equilibrio (ya calibrado con el pre-pase)
for i = 1:numel(r_span)
    cfg_r = cfg;
    cfg_r.fix_r   = 1;
    cfg_r.r_guess = r_span(i);
    cfg_r.rmin    = r_span(i);
    cfg_r.rmax    = r_span(i);
    % mantener B_mode y Bbar que ya usaste
    sol_r = solve_two_type_huggett_fiscal_Bfixed(cfg_r);

    ai = sol_r.a; gi = sol_r.g; da_i = ai(2)-ai(1);
    Ai = sum(gi(:,1).*ai)*da_i;
    Af = sum(gi(:,2).*ai)*da_i;
    Apriv_I(i) = Ai; 
    Apriv_F(i) = Af; 
    Apriv_T(i) = Ai + Af;
    B_grid(i)  = sol_r.fiscal.B;   % constante si B_mode='level'; variable si 'ratio_to_Y'
    Y_grid(i)  = sol_r.Y;
    S_grid(i)  = Apriv_T(i) - B_grid(i);
end

% ---- CSV de curvas
T_fin = table(r_span', Apriv_I', Apriv_F', Apriv_T', B_grid', Y_grid', S_grid', ...
    'VariableNames', {'r','A_I','A_F','A_priv','B_public','Y','Excess_S'});
writetable(T_fin, fullfile(outdir_tabs,'financial_market_curves.csv'));

% ---- Figura 1: A_I(r), A_F(r), A_priv(r) vs B(r)
fig = figure('Name','Financial market: private demand vs public supply');
plot(r_span, Apriv_I,'LineWidth',2); hold on;
plot(r_span, Apriv_F,'LineWidth',2);
plot(r_span, Apriv_T,'LineWidth',2);
plot(r_span, B_grid ,'k--','LineWidth',1.8);
xline(r,'r:','LineWidth',1.5);
grid on; xlabel('Interest rate r'); ylabel('Level');
if strcmpi(cfg.B_mode,'ratio_to_Y')
    legB = 'B(r) public supply';
else
    legB = 'B (public supply)';
end
legend({'A_I(r)','A_F(r)','A_{priv}(r)',legB,'r^*'},'Location','best');
title('Private demand by type vs Public bond supply');
export_fig(fig, fullfile(outdir_figs,'financial_market_private_vs_public'));

% ---- Figura 2: S(r)=A_priv(r)-B(r) (diagnóstico)
fig = figure('Name','Financial market: excess supply S(r)');
plot(r_span,S_grid,'LineWidth',2); hold on;
yline(0,'k--'); xline(r,'r:','LineWidth',1.5);
grid on; xlabel('r'); ylabel('S(r)=A_{priv}-B');
title('Excess supply of assets (market clearing at S=0)');
export_fig(fig, fullfile(outdir_figs,'financial_market_excess_supply'));

% ---- Figura 3: Descomposición al equilibrio (barras apiladas)
fig = figure('Name','Financial market: breakdown at r*');
X = categorical({'Private demand','Public supply B'});
X = reordercats(X, {'Private demand','Public supply B'});
Ybars = [A_I A_F 0; 0 0 fb.B]; % privado por tipo vs B
bar(X, Ybars, 'stacked'); grid on; ylabel('Level');
legend({'A_I (informal)','A_F (formal)','B (public)'},'Location','best');
title(sprintf('Composition at r^* = %.4f', r));
export_fig(fig, fullfile(outdir_figs,'financial_market_breakdown_at_rstar'));

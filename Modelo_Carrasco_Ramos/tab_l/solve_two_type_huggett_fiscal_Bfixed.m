...
% -------------------------------------------------------------------------
% Huggett 2 tipos con gobierno y mercado financiero.
% Cierres fiscales:
%   (A) 'G_resid_B_fixed'   : B exógeno, G residual  => BB=0
%   (B) 'G_fixed_B_endog'   : G fijo (nivel o ratio), B endógeno => BB=0
% Mercado financiero: r tal que A_priv(r) = B(r).
% -------------------------------------------------------------------------
...
fix_r   = arg('fix_r',0); maxit_r=arg('maxit_r',80); crit_S=arg('crit_S',1e-5);
...
psi_G=arg('psi_G',0.08); omegaG=arg('omegaG',0.50); report_G=arg('report_G_effects',0);

% HJB
maxit_V=arg('maxit_V',160); crit_V=arg('crit_V',1e-6); Delta=arg('Delta',1400);

% Gobierno: parámetros y modo
fiscal_mode = arg('fiscal_mode','G_resid_B_fixed'); % default tu cierre antiguo
B_mode  = arg('B_mode','level'); Bbar = arg('Bbar',0.25);
G_target_ratio = []; G_target_level = [];
if isfield(cfg,'G_target_ratio'), G_target_ratio = cfg.G_target_ratio; end
if isfield(cfg,'G_target_level'), G_target_level = cfg.G_target_level; end

% ============ Resolver r ============
if fix_r
    out = solve_given_r(r_guess); return;
else
    [Smin, ~] = eval_S(rmin);
    [Smax, ~] = eval_S(rmax);
    it=0;
    while Smin*Smax>0 && it<6
        rmin=max(0.003,0.7*rmin); rmax=1.4*rmax;
        [Smin,~]=eval_S(rmin); [Smax,~]=eval_S(rmax); it=it+1;
    end
    if Smin*Smax>0
        warning('Bisection: no sign change; return boundary.');
        if abs(Smin)<abs(Smax), out=solve_given_r(rmin); else, out=solve_given_r(rmax); end
        return;
    end
    rL=rmin; SL=Smin; rH=rmax; SH=Smax; best=[]; rM=r_guess;
    for k=1:maxit_r
        rM=0.5*(rL+rH);
        [SM, solM]=eval_S(rM);
        if isempty(best)||abs(SM)<abs(best.S_residual), best=solM; end
        if abs(SM)<crit_S, break; end
        if SL*SM<=0, rH=rM; SH=SM; else, rL=rM; SL=SM; end
    end
    out=best; return;
end
...
% ======= subfunciones =======
    ...
    function out_r = solve_given_r(r)
        ...
        Gpc=0;  % solo multiplica utilidad, no política
        for it=1:maxit_V
            Vprev=V;
        ...
        for kk=1:1000
            ...
            if norm(g_new-g,inf)<1e-12, break; end, g=g_new;
        end
        ...
        % ---- Fiscal ----
        Tl = tau_l*z2*popF; Tc = tau_c*Ctot; Tr = phi*z1*popI;

        switch lower(fiscal_mode)
            case 'g_resid_b_fixed'
                if strcmpi(B_mode,'ratio_to_Y'), B = Bbar*Y; else, B=Bbar; end
                rB = r*B; Gx = Tl + Tc - Tr - rB; PB = rB; BB = 0.0;

            case 'g_fixed_b_endog'
                if ~isempty(G_target_ratio)
                    Gx = G_target_ratio * Y;
                elseif ~isempty(G_target_level)
                    Gx = G_target_level;
                else
                    % fallback conservador: 8% del PIB
                    Gx = 0.08 * Y;
                end
                denom = max(r,1e-6);
                B = max(0, (Tl + Tc - Tr - Gx)/denom);
                rB = r*B;
                PB = Tl + Tc - Tr - Gx; BB = PB - rB; % ≈ 0 por construcción
...
% ------------------------ Helpers ------------------------
function v = get_arg(cfg,f,def), if isfield(cfg,f)&&~isempty(cfg.(f)), v=cfg.(f); else, v=def; end, end

...
% Fokker-Planck: esquema implícito en pseudo-tiempo (estable y sin warnings).
% -------------------------------------------------------------------------
...
% Difusión (estabilizador)
sigma_a = arg('sigma_a',0.007);  nu = 0.5*(sigma_a^2)/(da^2);

% Bien público en utilidad (multiplicativo)
...
% Gobierno y deuda
B_mode  = arg('B_mode','level');     % 'level' (recom.) | 'ratio_to_Y'
Bbar    = arg('Bbar',0.25);
gov_balance_mode = 'exact_g';        % fijo a exacto (limpio)
...
if fix_r
    ...
    if Smin*Smax > 0
        % No hubo cambio de signo: devuelve mejor borde (diagnóstico)
...
% ========================= Subfunciones locales ===========================
    ...
    function out_r = solve_given_r(r)
        ...
        for it=1:maxit_V
            ...
            % Generador A = drift + difusión + switching
        ...
        AT = A';
        g  = ones(2*I,1); g = g/(sum(g)*da);
        tauFP = 80*da;      % paso estable
        for k=1:1000
            g_new = (speye(2*I) - tauFP*AT) \ g;
            g_new = max(g_new,0);
            g_new = g_new/(sum(g_new)*da);
        ...
        out_r.r_path = r; out_r.S_path = S;           % diag simple

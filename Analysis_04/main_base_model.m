...
%   - r endógeno por clearing (Btarget = 0 baseline)
...
%% -------- Demanda/oferta de activos (bonos) por tipo (baseline)
...
title('Relación r(\eta) en el baseline');
set(gcf,'Color','w');

%% ===================== EXPORTAR CSVs =====================

% (A) Estadísticas de riqueza por simulación (jj=1..n_agents)
T_wealth = table( ...
    ...
    'VariableNames', {'eta','sI','sF','r', ...
                      ...
                      'gmean_tot','gmed_tot','gini_tot','p11'} );
...
% Fiscal{jj} = [Tc, Tl, Tr, G, rB, Gap, popI, popF, Y];
F = cell2mat(Fiscal(:));
T_fiscal = table( ...
    eta_vector(:), sI_vector1(:), sF_vector1(:), r_vec(:), ...
    F(:,1), F(:,2), F(:,3), F(:,4), F(:,5), F(:,6), F(:,7), F(:,8), F(:,9), ...
    'VariableNames', {'eta','sI','sF','r','Tc','Tl','Tr','G','rB','Gap','popI','popF','Y'} );
...
T_r_eta = table(pop_eta(:), r_eta(:), Fe(:,7), Fe(:,8), Fe(:,9), ...
    'VariableNames', {'eta','r','popI','popF','Y'});

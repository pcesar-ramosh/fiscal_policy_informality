...
%  - r endógeno por clearing (Btarget=0)
%
% Salidas extra:
...
statsMatrix  = nan(numel(pop1_vector), 16);
...
for jj = 1:numel(pop1_vector)
    ...
    gini_tot  = giniFromDist(a, g(:,1)+g(:,2));

    statsMatrix(jj,1:8) = [gmean_inf, gmean_for, gmed_inf, gmed_for, gmean_tot, gmed_tot, gini_tot, p11];
    ...
    % ---------- Resumen fiscal ----------
    C_tot = sum(c_inf .* g(:,1))*da + sum(c_for .* g(:,2))*da;
    Tc    = tau_c * C_tot;     
    Tl    = tau_l * z2 * popF; 
    TrAgg = phi   * z1 * popI; 
    Y     = z1*popI + z2*popF; 
    ...
    Fiscal{jj} = [Tc, Tl, TrAgg, G_agg, rB, Gap, popI, popF, Y];
end
...
function gini = giniFromDist(x, w)
x = x(:); w = w(:);
[xx, idx] = sort(x);
ww = w(idx);
W = sum(ww);
...
cumw = cumsum(ww);
cumxw = cumsum(xx .* ww);
L = cumxw / max(cumxw(end),eps);
gini = 1 - 2 * trapz(cumw, L);
end

...
cfg0.eta_target = 0.654;             % target informality (BASE)
cfg0.p22_bar   = 0.8155;
...
sc(2).name = 'INF_BAJA'; sc(2).cfg = cfg0; sc(2).cfg.eta_target = 0.25; % LOW informality
sc(3).name = 'INF_ALTA'; sc(3).cfg = cfg0; sc(3).cfg.eta_target = 0.75; % HIGH informality

pretty = @(nm) strrep(strrep(nm,'INF_BAJA','LOW INF'),'INF_ALTA','HIGH INF');
...
for k=1:numel(sc)
    ...
    fprintf('[%s] r*=%.4f | eta_real=%.3f | B=%.3f | G=%.3f | Y=%.3f | C=%.3f  (%.2fs)\n', ...
        sc(k).name, sc(k).sol.r, sc(k).sol.popI/(sc(k).sol.popI+sc(k).sol.popF), ...
        sc(k).sol.fiscal.B, sc(k).sol.fiscal.G, sc(k).sol.Y, sc(k).sol.Ctot, toc);
...
writetable(Tsum, fullfile(outdir_tabs,'compare_informality_summary.csv'));
disp(Tsum);
...
labels = [mk(' - I'), mk(' - F')];
legend([hI;hF], labels, 'Location','bestoutside');
title('Consumption policies (I solid, F dashed)');
export_fig(fig, fullfile(outdir_figs,'cmp_policy_consumption'));
...
export_fig(fig, fullfile(outdir_figs,'cmp_policy_savings'));
...
export_fig(fig, fullfile(outdir_figs,'cmp_wealth_distributions'));

% ---------- Asset market closure (stacked per scenario) ----------
...
for k=1:numel(sc)
    ...
    % numeric labels (totals above each stacked bar)
    xt = [1 2]; yt = [A_I+A_F, B];
    for j=1:2
        text(xt(j), yt(j), sprintf(' %.3f',yt(j)), 'HorizontalAlignment','center', ...
             'VerticalAlignment','bottom', 'FontSize',10);
    end
end
export_fig(fig, fullfile(outdir_figs,'cmp_asset_market_closure'));
...
for k=1:numel(sc)
    alt = sc(k).cfg; alt.fix_r=1; alt.maxit_r=1;
    ...
    hAp(k)=plot(r_span,Apriv,'LineWidth',2,'Color',cols(k,:));        % A_priv(r)
    hB(k)=plot(r_span,Bgrid,'--','LineWidth',1.6,'Color',cols(k,:));  % B(r)
    hr(k)=xline(sc(k).sol.r,':','Color',cols(k,:),'LineWidth',1.6);   % r*
end
...
export_fig(fig, fullfile(outdir_figs,'cmp_asset_demand_vs_B'));
...
export_fig(fig, fullfile(outdir_figs,'cmp_excess_supply_curve'));

% ---------- Fiscal (revenues/expenditures/balances) ----------
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'cmp_fiscal_accounts'));
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'cmp_borrowers_lenders'));

% ---------- Lorenz (wealth, total) with Gini in legend ----------
...
legend(hW, labels, 'Location','southeast');
title('Lorenz curve (Wealth, total)');
export_fig(fig, fullfile(outdir_figs,'cmp_lorenz_wealth_total'));

% ---------- Lorenz (wealth by type) ----------
fig=figure('Name','Lorenz wealth by type compare'); hold on;
hI = gobjects(numel(sc),1); hF = hI;
for k=1:numel(sc)
    [LwI,cI] = lorenz_from_assets_single(sc(k).sol.a, sc(k).sol.g(:,1));
    hI(k)=plot(cI, LwI, 'LineWidth',2, 'Color', cols(k,:),'LineStyle','-'); % Informal
end
for k=1:numel(sc)
    [LwF,cF] = lorenz_from_assets_single(sc(k).sol.a, sc(k).sol.g(:,2));
    hF(k)=plot(cF, LwF, 'LineWidth',2, 'Color', cols(k,:),'LineStyle','--'); % Formal
end
plot([0,1],[0,1],'k:'); grid on; axis square; xlabel('Population share'); ylabel('Wealth share');
...
export_fig(fig, fullfile(outdir_figs,'cmp_lorenz_wealth_bytype'));

% ---------- Lorenz (consumption, total) with Gini ----------
...
legend(hC, labels, 'Location','southeast');
title('Lorenz curve (Consumption, total)');
export_fig(fig, fullfile(outdir_figs,'cmp_lorenz_consumption_total'));

% ---------- Lorenz (consumption by type) ----------
...
for k=1:numel(sc)
    ...
    hI(k)=plot(cI, LcI, 'LineWidth',2, 'Color', cols(k,:), 'LineStyle','-'); % Informal
end
for k=1:numel(sc)
    [LcF,cF] = lorenz_from_values_single(sc(k).sol.c(:,2), sc(k).sol.g(:,2), sc(k).sol.a);
    hF(k)=plot(cF, LcF, 'LineWidth',2, 'Color', cols(k,:), 'LineStyle','--'); % Formal
end
plot([0,1],[0,1],'k:'); grid on; axis square; xlabel('Population share'); ylabel('Consumption share');
...
export_fig(fig, fullfile(outdir_figs,'cmp_lorenz_consumption_bytype'));
...
export_fig(fig, fullfile(outdir_figs,'cmp_mpc_by_assets'));

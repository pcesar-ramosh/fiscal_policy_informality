...
EXPORT_PDF        = false;     % true si quieres también PDF
...
% -------------------- Tax-hike scenarios --------------------------------
cfgVAT = cfg0; cfgVAT.tau_c = 0.30;     % VAT hike (consumption tax up)
cfgLAB = cfg0; cfgLAB.tau_l = 0.30;     % Labor-income tax hike

% --------------------------- Escenarios ----------------------------------
sc = struct('name',{},'cfg',{},'sol',{});
sc(1).name = 'BASE';     sc(1).cfg = cfg0;
sc(2).name = 'VAT_HIKE'; sc(2).cfg = cfgVAT;
sc(3).name = 'LAB_HIKE'; sc(3).cfg = cfgLAB;

pretty = @(nm) strrep(strrep(nm,'BASE','Current policy'), 'VAT_HIKE','VAT hike');
pretty = @(nm) strrep(pretty(nm),'LAB_HIKE','Labor tax hike');
...
writetable(Tsum, fullfile(outdir_tabs,'compare_tax_hikes_summary.csv'));
disp(Tsum);
...
export_fig(fig, fullfile(outdir_figs,'tax_policy_consumption'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_policy_savings'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_wealth_distributions'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_asset_market_closure'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_asset_demand_vs_B'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_excess_supply_curve'), EXPORT_PDF);
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'tax_fiscal_accounts'), EXPORT_PDF);
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'tax_borrowers_lenders'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_lorenz_wealth_total'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_lorenz_wealth_bytype'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_lorenz_consumption_total'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_lorenz_consumption_bytype'), EXPORT_PDF);
...
export_fig(fig, fullfile(outdir_figs,'tax_mpc_by_assets'), EXPORT_PDF);

function out = solve_two_type_huggett_no_fiscal_bytype(cfg, flags)
% -------------------------------------------------------------------------
% Dos tipos Huggett con posibilidad de "apagar" la política fiscal:
%   flags.zero_taxes       -> tau_l = tau_c = 0
%   flags.zero_transfers   -> phi = 0
%   flags.zero_public_good -> psi_G = 0   (sin efecto multiplicativo de G)
%   flags.zero_debt        -> B = 0
%
% El cierre sigue siendo A_priv(r) = B. Si B=0, el equilibrio r se ajusta
% para que la riqueza privada agregada sea 0 (economía "autárquica" en neto).
% -------------------------------------------------------------------------

% ----- Helpers
arg = @(f,def) get_arg(cfg,f,def);
on  = @(fld) (isfield(flags,fld) && islogical(flags.(fld)) && flags.(fld));

% Preferencias / impuestos base
sI=arg('RRA_I',3.40); sF=arg('RRA_F',3.40); rho=arg('rho',0.06);
tau_l0=arg('tau_l',0.15); tau_c0=arg('tau_c',0.18); phi0=arg('phi',0.09);

% Aplicar “apagado”
tau_l = on('zero_taxes')      * 0 + ~on('zero_taxes')      * tau_l0;
tau_c = on('zero_taxes')      * 0 + ~on('zero_taxes')      * tau_c0;
phi   = on('zero_transfers')  * 0 + ~on('zero_transfers')  * phi0;

% Ingresos
z1=arg('z1',0.33); z2=arg('z2',1.00);

% Bien público
psiG0=arg('psi_G',0.08); omegaG=arg('omegaG',0.50);
psi_G = on('zero_public_good') * 0 + ~on('zero_public_good') * psiG0;
...
% Gobierno / deuda
B_mode=arg('B_mode','ratio_to_Y'); Bbar0=arg('Bbar',0.80); alphaG=arg('alphaG',0.50);
clampG0=arg('clamp_G_to_zero',true); G_cap_ratio=arg('G_cap_ratio',1.0);

% Si apagamos deuda => B=0 (independiente de Y)
B_mode_eff = B_mode; Bbar_eff = Bbar0;
if on('zero_debt')
    B_mode_eff = 'level'; Bbar_eff = 0.0;
end
...
V=zeros(I,2);
c_inf_min=max((z1 + phi*z1 + rr1(1)*a(1))/(1+tau_c),1e-12);
c_for_min=max(((1-tau_l)*z2 + rr2(1)*a(1))/(1+tau_c),1e-12);
V(:,1)=u_CRRA(c_inf_min,sI)/rho; V(:,2)=u_CRRA(c_for_min,sF)/rho; v0=V;
...
for ir=1:Ir
    ...
    for it=1:maxit_V
        ...
        % fronteras
        c_inf_max=max((z1 + phi*z1 + rr1(end)*a(end))/(1+tau_c),1e-12);
    ...
    % Fiscal
    if strcmpi(B_mode_eff,'ratio_to_Y'), B=Bbar_eff*Y; else, B=Bbar_eff; end
    rB=r*B; Tl=tau_l*z2*popF; Tc=tau_c*Ctot; Tr=phi*z1*popI;

    if on('zero_public_good')
        G_resid = 0.0;
    else
        G_resid = Tl + Tc - Tr - rB;
    ...
    if ~fix_r
        ...
        rr1=r*ones(I,1); rr1(a<0)=r+theta_I;
        rr2=r*ones(I,1); rr2(a<0)=r+theta_F;
        V(:,1)=u_mult(max((z1 + phi*z1 + rr1.*a)/(1+tau_c),1e-12),0,sI,psi_G,omegaG)/rho;
...
rr1=r*ones(I,1); rr1(a<0)=r+theta_I;
rr2=r*ones(I,1); rr2(a<0)=r+theta_F;
s_pol=[ z1 + phi*z1 + rr1.*a - (1+tau_c)*c(:,1), ...
        (1-tau_l)*z2 + rr2.*a - (1+tau_c)*c(:,2) ];

% Estadísticos (por tipo y total)

...
% -------------------- Escenarios: choques y transferencias ----------------
pol(1) = struct('name','BASE',        'shockI',0.00,'shockF',0.00,'xi_I',0.00,'xi_F',0.00);
pol(2) = struct('name','SHOCK_noTR',  'shockI',0.10,'shockF',0.05,'xi_I',0.00,'xi_F',0.00);
pol(3) = struct('name','SHOCK_TR',    'shockI',0.10,'shockF',0.05,'xi_I',0.70,'xi_F',0.30);

SC = struct('name',{},'cfg',{},'pol',{},'sol',{});
for k=1:numel(pol)
    SC(k).name = pol(k).name;
    SC(k).cfg  = cfg0;
    SC(k).pol  = pol(k);
end
...
for k=1:numel(SC)
    tic;
    SC(k).sol = solve_two_type_huggett_fiscal_transfer_shock_bytype(SC(k).cfg, SC(k).pol);
    s=SC(k).sol;
    ...
    f=SC(k).sol.fiscal;
    T_fiscal = table(scen, f.Tl, f.Tc, f.Tr, f.G, f.rB, f.PB, f.B, f.BB, f.tI_pc, f.tF_pc, ...
...
labelsIF = labels_by_order(names);  % en el orden de trazado: (1-I,1-F,2-I,2-F,...)

% ---------- (1) Consumo por tipo: c(a)
...
export_fig(fig, fullfile(outdir_figs,'bytype_policy_consumption'));

% ---------- (2) Ahorro por tipo: s(a)
...
legend(labelsIF, 'Location','bestoutside');
export_fig(fig, fullfile(outdir_figs,'bytype_policy_savings'));
...
export_fig(fig, fullfile(outdir_figs,'bytype_lorenz_wealth_total'));
...
export_fig(fig, fullfile(outdir_figs,'bytype_lorenz_consumption_total'));
...
for k=1:numel(SC)
    T.idx(k)=k;
    T.gW_I(k)=SC(k).sol.stats.giniW(1);
    T.gW_F(k)=SC(k).sol.stats.giniW(2);
    T.gC_I(k)=SC(k).sol.stats.giniC(1);
    T.gC_F(k)=SC(k).sol.stats.giniC(2);
end
...
set(gca,'XTick',1:numel(SC),'XTickLabel',names); grid on; ylabel('Gini');
legend({'Informal','Formal'},'Location','best'); xtickangle(20);
export_fig(fig, fullfile(outdir_figs,'bytype_ginis'));

% ---------- (6) Fiscal: ingresos-gastos-balances (leyendas claras)
fig=figure('Name','Fiscal accounts (labels fixed)');
...
export_fig(fig, fullfile(outdir_figs,'bytype_fiscal_accounts'));
...
legend(names,'Location','best'); add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'bytype_borrowers_lenders'));

% ---------- (8) MPC(a) por tipo (r fijo en cada escenario)
fig=figure('Name','MPC(a) by type'); hold on;
for k=1:numel(SC)
    cfg=SC(k).cfg; r=SC(k).sol.r; a=SC(k).sol.a; pol=SC(k).pol;
    alt=cfg; alt.fix_r=1; alt.r_guess=r; alt.rmin=r; alt.rmax=r;
    eps=0.01; alt.z1=cfg.z1*(1+eps); alt.z2=cfg.z2*(1+eps);
    solP = solve_two_type_huggett_fiscal_transfer_shock_bytype(alt, pol);
...
export_fig(fig, fullfile(outdir_figs,'bytype_mpc'));
...
for k=1:numel(SC)
    ...
    for i=1:numel(r_span)
        ...
        tmp=solve_two_type_huggett_fiscal_transfer_shock_bytype(alt, pol);
...
export_fig(fig, fullfile(outdir_figs,'bytype_asset_demand_vs_B'));
...
export_fig(fig, fullfile(outdir_figs,'bytype_excess_supply'));

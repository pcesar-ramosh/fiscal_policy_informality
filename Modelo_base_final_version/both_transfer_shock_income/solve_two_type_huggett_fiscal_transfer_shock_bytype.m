function out = solve_two_type_huggett_fiscal_transfer_shock_bytype(cfg, pol)
% -------------------------------------------------------------------------
% Dos tipos Huggett con B exógeno. Choque a ingresos (I,F) y transferencias
% per cápita por tipo. Devuelve stats por tipo y total, con Gini por tipo.
% -------------------------------------------------------------------------

% ----- Helpers
arg = @(f,def) get_arg(cfg,f,def);

% Preferencias / impuestos
sI=arg('RRA_I',3.40); sF=arg('RRA_F',3.40); rho=arg('rho',0.06);
tau_l=arg('tau_l',0.15); tau_c=arg('tau_c',0.18); phi=arg('phi',0.09);

% Ingresos y choques
z1b=arg('z1',0.33); z2b=arg('z2',1.00);
shockI=safe(pol,'shockI',0); shockF=safe(pol,'shockF',0);
z1=(1-shockI)*z1b; z2=(1-shockF)*z2b;

% Top-up per cápita
xi_I=safe(pol,'xi_I',0); xi_F=safe(pol,'xi_F',0);
tI_pc = xi_I * (shockI * z1b);
tF_pc = xi_F * (shockF * z2b);

% Primas
theta_I=arg('theta_I',0.06); theta_F=arg('theta_F',0.01);

% Activos
I=arg('I',700); amin=arg('amin',-2.0*z1b); amax=arg('amax',3.0);
...
for ir=1:Ir
    ...
    rB=r*B; Tl=tau_l*z2*popF; Tc=tau_c*Ctot;
...
function v = safe(s, f, def), if isfield(s,f) && ~isempty(s.(f)), v=s.(f); else, v=def; end, end

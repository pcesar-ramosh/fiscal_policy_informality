function out = solve_two_type_huggett_fiscal_transfer_shock(cfg, pol)
% -------------------------------------------------------------------------
% Dos tipos Huggett con B exógeno. Choque a ingresos por tipo y
% transferencias per cápita por tipo (top-up) para amortiguar.
%
% pol.shockI ∈ [0,1], pol.shockF ∈ [0,1]   (caída proporcional de z1,z2)
% pol.xi_I, pol.xi_F ∈ [0,1]               (fracción de compensación)
%
% Recursos:
%   Informal:  z1' + phi*z1' + tI_pc + r^theta a
%   Formal:    (1-tau_l) z2' + tF_pc + r^theta a
% Fiscal:
%   Tl = tau_l*z2'*popF;  Tc = tau_c*Ctot;
%   Tr = phi*z1'*popI + tI_pc*popI + tF_pc*popF;
%   G residual (con clamps y cap), B exógeno y rB=r*B.
% -------------------------------------------------------------------------

% ---------- Helpers y parámetros base
...
% Ingresos base y choques
z1_base = arg('z1',0.33); z2_base = arg('z2',1.00);
shockI = 0.0; shockF = 0.0;
if isfield(pol,'shockI'), shockI = max(0,min(1,pol.shockI)); end
if isfield(pol,'shockF'), shockF = max(0,min(1,pol.shockF)); end
z1 = (1 - shockI) * z1_base;
z2 = (1 - shockF) * z2_base;

% Top-up per cápita por tipo
xi_I = 0.0; xi_F = 0.0;
if isfield(pol,'xi_I'), xi_I = max(0,min(1,pol.xi_I)); end
if isfield(pol,'xi_F'), xi_F = max(0,min(1,pol.xi_F)); end
tI_pc = xi_I * (shockI * z1_base);   % monto per cápita informal
tF_pc = xi_F * (shockF * z2_base);   % monto per cápita formal
...
% Activos
I    = arg('I',700);
amin = arg('amin',-2.0*z1_base); amax = arg('amax',3.0);
...
V = zeros(I,2);
% Recursos en fronteras con transferencias per cápita
c_inf_max = max((z1 + phi*z1 + rr1(end)*a(end) + tI_pc)/(1+tau_c), 1e-12);
c_for_max = max(((1-tau_l)*z2 + rr2(end)*a(end) + tF_pc)/(1+tau_c), 1e-12);
c_inf_min = max((z1 + phi*z1 + rr1(1)*a(1)  + tI_pc)/(1+tau_c), 1e-12);
c_for_min = max(((1-tau_l)*z2 + rr2(1)*a(1)  + tF_pc)/(1+tau_c), 1e-12);

V(:,1) = u_CRRA(c_inf_min, sI)/rho;
V(:,2) = u_CRRA(c_for_min, sF)/rho;
v0 = V;
...
for ir=1:Ir
    ...
    for it=1:maxit_V
        ...
        % Fronteras (recalcular en cada iter)
        c_inf_max = max((z1 + phi*z1 + rr1(end)*a(end) + tI_pc)/(1+tau_c), 1e-12);
        c_for_max = max(((1-tau_l)*z2 + rr2(end)*a(end) + tF_pc)/(1+tau_c), 1e-12);
        c_inf_min = max((z1 + phi*z1 + rr1(1)*a(1)  + tI_pc)/(1+tau_c), 1e-12);
        c_for_min = max(((1-tau_l)*z2 + rr2(1)*a(1)  + tF_pc)/(1+tau_c), 1e-12);
        ...
        % Recursos (antes de c)
        res_inf = z1 + phi*z1 + tI_pc + rr1.*a;
        res_for = (1-tau_l)*z2 + tF_pc + rr2.*a;
    ...
    S = A_priv - B; r_path=[r_path; r]; S_path=[S_path; S];
    ...
    if ~fix_r
        ...
        % reset v0 para nuevo r
        rr1=r*ones(I,1); rr1(a<0)=r+theta_I;
        rr2=r*ones(I,1); rr2(a<0)=r+theta_F;
        V(:,1)=u_mult(max((z1 + phi*z1 + rr1.*a + tI_pc)/(1+tau_c),1e-12),0,sI,psi_G,omegaG)/rho;
        V(:,2)=u_mult(max(((1-tau_l)*z2 + rr2.*a + tF_pc)/(1+tau_c),1e-12),0,sF,psi_G,omegaG)/rho;
        v0=V;
    else
        break;
    end
...
rr1=r*ones(I,1); rr1(a<0)=r+theta_I;
rr2=r*ones(I,1); rr2(a<0)=r+theta_F;
s_pol=[ z1 + phi*z1 + tI_pc + rr1.*a - (1+tau_c)*c(:,1), ...
        (1-tau_l)*z2 + tF_pc + rr2.*a - (1+tau_c)*c(:,2) ];
...
out.params.shock = struct('z1_drop',shockI,'z2_drop',shockF,'xi_I',xi_I,'xi_F',xi_F);

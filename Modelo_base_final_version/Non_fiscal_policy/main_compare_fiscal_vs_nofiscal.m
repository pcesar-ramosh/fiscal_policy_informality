% ========== main_compare_fiscal_vs_nofiscal.m ==========
clear; clc; close all;
...
% ----------------------- Baseline config (WITH fiscal) -------------------
cfgF = struct();
cfgF.RRA_I = 3.40; cfgF.RRA_F = 3.40;
cfgF.rho   = 0.08;
cfgF.tau_l = 0.15; cfgF.tau_c = 0.18; cfgF.phi = 0.09;
cfgF.z1 = 0.33; cfgF.z2 = 1.00;

cfgF.theta_I = 0.06;  % informal premium
cfgF.theta_F = 0.01;  % formal premium

cfgF.eta_target = 0.654;     % target informality
cfgF.p22_bar   = 0.8155;

cfgF.I = 700; cfgF.amax = 3.0; cfgF.amin = -2.0*cfgF.z1;

cfgF.r_guess = 0.03; cfgF.rmin = 0.005; cfgF.rmax = 0.10;
cfgF.maxit_V = 140; cfgF.crit_V = 1e-6; cfgF.Delta = 1400;
cfgF.maxit_r = 1500; cfgF.crit_S = 1e-5; cfgF.fix_r = 0;

% Public good (mild, as per last refinements)
cfgF.psi_G   = 0.08;
cfgF.omegaG  = 0.50;
cfgF.report_G_effects = 0;

% Diffusion
cfgF.sigma_a = 0.010;

% Government / Debt
cfgF.B_mode  = 'ratio_to_Y';   % ratio B/Y
cfgF.Bbar    = 0.35;           % B/Y
cfgF.alphaG  = 0.50;
cfgF.clamp_G_to_zero = true;
cfgF.G_cap_ratio = 0.08;

% ---------------- Scenario WITHOUT fiscal policy -------------------------
cfgN = cfgF;
cfgN.tau_l = 0; cfgN.tau_c = 0; cfgN.phi = 0;  % no taxes, no transfers
cfgN.psi_G = 0; cfgN.omegaG = 0;               % no public good in utility
cfgN.B_mode = 'level'; cfgN.Bbar = 0;          % B = 0 level -> no public debt
cfgN.G_cap_ratio = 0;                           % no G cap needed (G=0 anyway)

% --------------------------- Scenarios -----------------------------------
sc = struct('name',{},'cfg',{},'sol',{});
sc(1).name = 'WITH_FISCAL'; sc(1).cfg = cfgF;
sc(2).name = 'NO_FISCAL';   sc(2).cfg = cfgN;

pretty = @(nm) strrep(strrep(nm,'WITH_FISCAL','Current policy'), 'NO_FISCAL','No fiscal policy');
...
writetable(Tsum, fullfile(outdir_tabs,'compare_fiscal_vs_nofiscal_summary.csv'));
disp(Tsum);
...
export_fig(fig, fullfile(outdir_figs,'fvn_policy_consumption'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_policy_savings'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_wealth_distributions'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_asset_market_closure'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_asset_demand_vs_B'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_excess_supply_curve'));
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'fvn_fiscal_accounts'));
...
add_grouped_labels(h,'%.3f');
export_fig(fig, fullfile(outdir_figs,'fvn_borrowers_lenders'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_lorenz_wealth_total'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_lorenz_wealth_bytype'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_lorenz_consumption_total'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_lorenz_consumption_bytype'));
...
export_fig(fig, fullfile(outdir_figs,'fvn_mpc_by_assets'));

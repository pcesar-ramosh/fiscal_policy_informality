...
% para A_priv = B. Registra r_path y S_path (bisección).
% -------------------------------------------------------------------------
...
% === NUEVO: trayectorias de bisección ===
r_path = []; S_path = [];
...
for ir=1:Ir
    ...
    if ~isempty(G_cap_ratio) && G_cap_ratio>0
        G_resid = min(G_resid, G_cap_ratio*Y);
    end
    ...
    % === NUEVO: guardar trayectoria de bisección ===
    r_path = [r_path; r];
    S_path = [S_path; S];

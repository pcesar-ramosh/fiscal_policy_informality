...
% -------------------------------------------------------------------------
% Huggett 2 tipos con B exógena (nivel o ratio a Y), cierre fiscal vía
% bien público G>=0 que entra multiplicativamente en utilidad (afecta FOC),
% primas por tipo (theta_I > theta_F), difusión en activos y búsqueda en r.
% Salidas incluyen descomposición A_priv por tipo, Lorenz/Gini, diagnósticos.
% -------------------------------------------------------------------------

arg = @(f,def) (isfield(cfg,f) && ~isempty(cfg.(f)))*cfg.(f) + ...
               ~(isfield(cfg,f) && ~isempty(cfg.(f)))*def;

% Preferencias, impuestos e ingresos
...
% Primas por tipo
theta_I=arg('theta_I',0.08); theta_F=arg('theta_F',0.02);

% Grid de activos
I=arg('I',700); amax=arg('amax',5.0); amin=arg('amin',-0.30*z1);
...
% Ocupación (hazards constantes que replican eta_target)
...
% Difusión para regularizar g(a)
sigma_a=arg('sigma_a',0.012); nu=0.5*(sigma_a^2)/(da^2);

% ---- Bien público multiplicativo en utilidad (afecta FOC) ----
psi_G = arg('psi_G',0.30);   % intensidad
omegaG= arg('omegaG',1.0);   % elasticidad

% Esquema numérico HJB
maxit_V=arg('maxit_V',140); crit_V=arg('crit_V',1e-6); Delta=arg('Delta',1400);

% Fisco: B exógena y G cierra
B_mode = arg('B_mode','ratio_to_Y');         % 'level' | 'ratio_to_Y'
Bbar   = arg('Bbar',0.35);
alphaG = arg('alphaG',0.50);
clampG0= arg('clamp_G_to_zero',true);
G_cap_ratio=arg('G_cap_ratio',0.08);         % cap opcional: G <= γ Y

% ---- v0 (Gpc=0) con primas por tipo
...
Gx=0; Gx_old=0; Gpc=0; S=NaN; A=[]; Tl=0; Tc=0; Tr=0; rB=0; PB=0; BB=0; B=0;
...
for ir=1:Ir
    ...
    for it=1:maxit_V
        ...
        cI_max=max((z1+rr1(end)*a(end)+phi*z1)/(1+tau_c),1e-12);
        cF_max=max(((1-tau_l)*z2+rr2(end)*a(end))/(1+tau_c),1e-12);
        cI_min=max((z1+rr1(1)*a(1)+phi*z1)/(1+tau_c),1e-12);
        cF_min=max(((1-tau_l)*z2+rr2(1)*a(1))/(1+tau_c),1e-12);
        dVf(I,1)=(1+tau_c)*facI*u_CRRA_prime(cI_max,sI);
        dVf(I,2)=(1+tau_c)*facF*u_CRRA_prime(cF_max,sF);
        dVb(1,1)=(1+tau_c)*facI*u_CRRA_prime(cI_min,sI);
        dVb(1,2)=(1+tau_c)*facF*u_CRRA_prime(cF_min,sF);

        res_inf=z1+rr1.*a+phi*z1;       % antes de c
    ...
    G_resid=Tl + Tc - Tr - rB; if clampG0, G_resid=max(0,G_resid); end
    ...
    % ===== Cierre financiero =====
    A_I = g(:,1)'*a*da; A_F = g(:,2)'*a*da; A_priv = A_I + A_F;
    S = A_priv - B;
...
% Políticas de ahorro
rr1=r*ones(I,1); rr1(a<0)=r+theta_I; rr2=r*ones(I,1); rr2(a<0)=r+theta_F;
...
volBorrow_I =sum(g(idxB,1).*a(idxB))*da; volBorrow_F=sum(g(idxB,2).*a(idxB))*da;
volLend_I   =sum(g(idxL,1).*a(idxL))*da; volLend_F  =sum(g(idxL,2).*a(idxL))*da;
...
out.Apriv = A_priv; out.Apriv_I=A_I; out.Apriv_F=A_F;

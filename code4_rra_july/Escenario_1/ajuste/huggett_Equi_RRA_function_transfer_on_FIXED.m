function [r_opt, ir, pop1_vector, a, g_opt, c_opt] = ...
    huggett_Equi_RRA_function_transfer_on_FIXED(eta_vector, sI_vector1, sF_vector1, cfg)
% HUGGETT_EQUI_RRA_FUNCTION_TRANSFER_ON_FIXED
% Cambios clave:
%  - Transferencia entra al PRESUPUESTO del informal (no a utilidad)
%  - Impuesto al consumo en la FOC: c = [(1+tauc)*V_a]^(-1/sigma)
%  - Sin "Gov" en utilidad
%  - Delta reducido (mejor condición)
%  - Fokker-Planck: normalización sum(g)*da = 1
%  - Auto-expansión del bracket si r* cae en el borde
...
for jj = 1:J
    ...
    maxit = 100;       crit = 1e-6;
    Delta = 0.500;        % más diagonal, mejor condición numérica
    Ir    = 100;       crit_S = 1e-5;
    ...
    % Valor inicial V0 (consumo sin ahorro; Transfer solo informales)
    ...
    % ===== Encontrar r* por bisección (con auto-expansión si es necesario) =====
    MAX_EXPAND = 3;  expand_count = 0;
    converged_r = false;
    ...
    while ~converged_r
        for ir = 1:Ir
            ...
            for n = 1:maxit
                ...
                % Recursos (Transfer SOLO en informales)
                res_I = (1 - taxI)*z(1) + rr_vec.*aa(:,1) + Transfer;
                res_F = (1 - taxF)*z(2) + r*aa(:,2);
                ...
                % Utilidades (sin Gov, sin Transfer en gusto)
            ...
            % ===== Fokker-Planck (normalización directa) =====
            AT = A';
            bb = zeros(2*I,1);
            AT(1,:) = ones(1, 2*I) * da;  % suma de probabilidades = 1
            bb(1)   = 1;
        ...
        if ~converged_r
            % Si cayó al borde, intenta expandir el bracket de forma controlada
            at_lower = abs(r - rmin) < 1e-12;
            at_upper = abs(r - rmax) < 1e-12;

            if (at_lower || at_upper) && expand_count < MAX_EXPAND
                if S > 0
                    % exceso de ahorro => baja más rmin
                    rmin = rmin - 0.02;
                else
                    % exceso de deuda => sube rmax
                    rmax = rmax + 0.02;
                end

...
%  Equilibrio en un modelo HA con informalidad, transfer focalizada,
%  y shock COVID (ingresos). Retro-compatible.
%
% Entradas:
%   - eta_vector   : 1xJ, tamaño del sector informal (mismo valor repetido)
%   - sI_vector1   : 1xJ, RRA informal por agente
%   - sF_vector1   : 1xJ, RRA formal por agente
%   - cfg (struct) :
%       scenario             = "baseline" | "covid"
%#      psi_I, psi_F         : multiplicadores de ingreso (default 1,1)
%       keep_transfers_level = true -> Transfer = phi*z1_base
%       transfer_multiplier  = 1.0 (kappa, usualmente 1 en este script)
%       amin_mode            = "baseline" | "shocked" (límite inferior)
%       gov_nonneg           = true (truncar bien público a >= 0)
%       phi                  = 0.13
%       taxF, taxI, tauc     = tasas (tauc no implementada en gasto)
%       theta                = 0.02
%       r0, rmin, rmax       = bracket para r*
%
% Salidas:
%   r_opt(1xJ), ir (iter alcanzada), pop1_vector (=eta_vector), a (Ix1),
%   g_opt{1xJ}(Ix2), c_opt{1xJ}(Ix2)
...
for jj = 1:J
    ...
    for i = 1:I
        if a(i) >= 0
            rr(i) = r;         % prestar al r de mercado
        else
            rr(i) = r + cfg.theta; % endeudarse con prima informal
        end
    end

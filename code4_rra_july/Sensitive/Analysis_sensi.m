...
%     huggett_Equi_RRA_function_transfer_sensi(eta_vector,sI_vector1,sF_vector1);

% Caso 1: Sin transferencias
phi_0 = 0   ;
[r_opt0, ir0, pop1_vector0, a0, g_opt0, c_opt0] = ...
    huggett_Equi_RRA_function_transfer_sensi(eta_vector, sI_vector1, sF_vector1, phi_0);

% Caso 2: Con transferencias del 6%
phi_6 = 0.13;
[r_opt6, ir6, pop1_vector6, a6, g_opt6, c_opt6] = ...
    huggett_Equi_RRA_function_transfer_sensi(eta_vector, sI_vector1, sF_vector1, phi_6);

%%

j = 10; % elige el agente j a comparar

figure;

subplot(1,2,1)
plot(a0, c_opt0{j}(:,1), 'r-', 'LineWidth', 1.5); hold on;
plot(a6, c_opt6{j}(:,1), 'r--', 'LineWidth', 1.5);
plot(a0, c_opt0{j}(:,2), 'b-', 'LineWidth', 1.5);
plot(a6, c_opt6{j}(:,2), 'b--', 'LineWidth', 1.5);
xlabel('Activos (a)'); ylabel('Consumo');
legend('Inf  φ=0.00', 'Inf  φ=0.13', 'Form  φ 0.00', 'Form  φ=0.13');
title(['Optimal Consumption']);
grid on;

subplot(1,2,2)
plot(a0, g_opt0{j}(:,1), 'r-', 'LineWidth', 1.5); hold on;
plot(a6, g_opt6{j}(:,1), 'r--', 'LineWidth', 1.5);
plot(a0, g_opt0{j}(:,2), 'b-', 'LineWidth', 1.5);
plot(a6, g_opt6{j}(:,2), 'b--', 'LineWidth', 1.5);
xlabel('Activos (a)'); ylabel('Distribución de riqueza');
legend('Inf  φ=0.10', 'Inf  φ=0.13', 'Form  φ 0.10', 'Form  φ=0.13');
title(['Comparación distribución g(a) - RRA informal #' num2str(j)]);
grid on;



%% 
% === Different fiscal transfer cases ===
phi_11 = 0.11;
phi_12 = 0.12;
phi_13 = 0.13;

[r_opt11, ir11, pop1_vector11, a11, g_opt11, c_opt11] = ...
    huggett_Equi_RRA_function_transfer_sensi(eta_vector, sI_vector1, sF_vector1, phi_11);

[r_opt12, ir12, pop1_vector12, a12, g_opt12, c_opt12] = ...
    huggett_Equi_RRA_function_transfer_sensi(eta_vector, sI_vector1, sF_vector1, phi_12);

[r_opt13, ir13, pop1_vector13, a13, g_opt13, c_opt13] = ...
    huggett_Equi_RRA_function_transfer_sensi(eta_vector, sI_vector1, sF_vector1, phi_13);

%% Choose the agent j to compare
j = 10;
...
figure;
subplot(1,2,1)
plot(a11, c_opt11{j}(:,1), 'r-', 'LineWidth', 1.5); hold on;
plot(a12, c_opt12{j}(:,1), 'r--', 'LineWidth', 1.5);
plot(a13, c_opt13{j}(:,1), 'r:', 'LineWidth', 1.5);

plot(a11, c_opt11{j}(:,2), 'b-', 'LineWidth', 1.5);
plot(a12, c_opt12{j}(:,2), 'b--', 'LineWidth', 1.5);
plot(a13, c_opt13{j}(:,2), 'b:', 'LineWidth', 1.5);

xlabel('Assets (a)');
ylabel('Consumption');
legend('Informal ϕ=0.11','Informal ϕ=0.12','Informal ϕ=0.13', ...
       'Formal ϕ=0.11','Formal ϕ=0.12','Formal ϕ=0.13', ...
       'Location', 'southeast');
title(['Optimal Consumption - Agent #' num2str(j)]);
grid on;

%% Plot: Wealth Distribution

subplot(1,2,2)
plot(a11, g_opt11{j}(:,1), 'r-', 'LineWidth', 1.5); hold on;
plot(a12, g_opt12{j}(:,1), 'r--', 'LineWidth', 1.5);
plot(a13, g_opt13{j}(:,1), 'r:', 'LineWidth', 1.5);

plot(a11, g_opt11{j}(:,2), 'b-', 'LineWidth', 1.5);
plot(a12, g_opt12{j}(:,2), 'b--', 'LineWidth', 1.5);
plot(a13, g_opt13{j}(:,2), 'b:', 'LineWidth', 1.5);
...
legend('Informal ϕ=0.11','Informal ϕ=0.12','Informal ϕ=0.13', ...
       'Formal ϕ=0.11','Formal ϕ=0.12','Formal ϕ=0.13', ...
       'Location', 'northeast');
title(['Wealth Distribution g(a) - Agent #' num2str(j)]);
grid on;

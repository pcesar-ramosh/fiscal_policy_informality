...
cfg_base.psi_F                 = 1.00;        % sin shockg
...
%% --------------------------------------------------------------
%  Comparaciones clave (r* y fracción en restricción)
%% --------------------------------------------------------------
...
%% --------------------------------------------------------------
% Elegimos el "agente típico" como el más cercano a la mediana de sI
[~, j_star] = min(abs(sI_vector1 - median(sI_vector1)));
% (Opcional) índice fijo:
% j_star = ceil(n_agents/2);

% Preparar datos
a_base = a_b;  a_covid = a_c;                 % mallas de activos
c_base = c_b{j_star};  c_covid = c_c{j_star}; % consumo [I x 2]
g_base = g_b{j_star};  g_covid = g_c{j_star}; % densidades [I x 2]

% Limpieza visual de g(a)
...
% ------------------- Panel (2): Distribución de riqueza -------------
...
if export_excel
    ...
    % (2) fracción en restricción (informal/formal)
    ...
    % (3) Series del agente típico (consumo y g(a))
    Ta = table(a_base, a_covid, 'VariableNames', {'a_base','a_covid'});
    Tc = table(c_base(:,1), c_base(:,2), c_covid(:,1), c_covid(:,2), ...
        'VariableNames', {'c_inf_base','c_for_base','c_inf_covidTup','c_for_covidTup'});
    Tg = table(g_base(:,1), g_base(:,2), g_covid(:,1), g_covid(:,2), ...
        'VariableNames', {'g_inf_base','g_for_base','g_inf_covidTup','g_for_covidTup'});

    writetable(Ta, filename, 'Sheet', 'typical_a');
    writetable(Tc, filename, 'Sheet', 'typical_c');
    writetable(Tg, filename, 'Sheet', 'typical_g');

    fprintf('Exportado a: %s\n', filename);
end

disp('Listo: Benchmark vs. COVID (Transfers↑) resueltos, graficados y configurados para exportar (off).');
...
%% --------------------------------------------------------------
export_excel = true;   % <- déjalo en true para exportar
if export_excel
    filename = 'summary_covid_uptransfer.xlsx';

    % ---------- r* y fracción en restricción (ya calculados arriba) ----------
    % r* por agente
    T_r = table((1:n_agents)', r_b(:), r_c(:), 'VariableNames', ...
        {'agent','r_base','r_covid_Tup'});
    writetable(T_r, filename, 'Sheet', 'r_star');

    % fracción en restricción (si no las tienes, vuelve a calcular)
    if ~exist('frac_const_base','var') || ~exist('frac_const_cvd','var')
        frac_const_base = zeros(n_agents, 2);
        frac_const_cvd  = zeros(n_agents, 2);
        for j = 1:n_agents
            Gb = g_b{j}; Gc = g_c{j};
    ...
    % Malla y paso (misma para base/covid en tu set-up)
    a_grid = a_b;
    ...
    for j = 1:n_agents
        ...
        % (ii) Consumo por ocupación — usa pesos condicionales en cada ocupación
        C_base = c_b{j};                      % I x 2
        % Informales
        x_c_inf_b = C_base(:,1);
